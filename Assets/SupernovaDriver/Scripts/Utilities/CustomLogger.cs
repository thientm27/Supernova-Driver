...
namespace SupernovaDriver.Scripts.Utilities
{
    public static class DLogger
    {
                    ...
                    public static void Log(string message, string tag = "QLogger", string color = "#FFFFFF")
                    {
                        Debug.LogFormat("<color={2}>[{0}]</color> {1}", tag, message, color);
                    }
                    // ReSharper disable Unity.PerformanceAnalysis
                    public static void LogWarning(string message, string tag = "QLogger", string color = "#FFFFFF")
                    {
                        Debug.LogWarningFormat("<color={2}>[{0}]</color> {1}", tag, message, color);
                    }
                    // ReSharper disable Unity.PerformanceAnalysis
                    public static void LogError(string message, string tag = "QLogger", string color = "#FFFFFF")
                    {
